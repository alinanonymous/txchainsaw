public class Content {
    public static String introduction = """
            \u001b[HIt is Autumn of 1972.
            Tragedy strikes 19 year old Rose Taylor on a drive home with her boyfriend,
            22 year old Johnny Sawyer. After being abandoned at a gas station by her
            false lover, a relaxing night on the town would become a nightmare that was
            all too real.
                The worst things she could have imagined do not compare to the horrors
            she would experience during the events of this night and the subsequent days.
            The case of Rose Taylor's misfortunate suffering and anguish would be
            documented as yet another story of the bizarre crimes that were part of:
                """;
    public static String instructions = """
            You, the player, are Rose Taylor. Your boyfriend, Johnny Sawyer, was
            unfortunately from a family of cannibalistic serial murderers. He has left you
            in the basement of the Sawyer estate, unconscious. You do not know what
            this house has in store for you, but you know you must escape some way.
            Will you survive? What will be left of you?


            The house is made of an almost labyrinthine series of rooms.
            You may type \u001b[38;5;196mgo\u001b[0m, then the following directions to move:

            \u001b[38;5;196mn\u001b[0m or \u001b[38;5;196mnorth\u001b[0m to move north
            \u001b[38;5;196ms\u001b[0m or \u001b[38;5;196msouth\u001b[0m to move south
            \u001b[38;5;196me\u001b[0m or \u001b[38;5;196meast\u001b[0m to move east
            \u001b[38;5;196mw\u001b[0m or \u001b[38;5;196mwest\u001b[0m to move west

            There are also items and interactables around the house. The following
            commands can be used for interaction with your environment:

            \u001b[38;5;196muse\u001b[0m to interact with something in your surroundings
            \u001b[38;5;196mexam\u001b[0m to examine an object closer
            \u001b[38;5;196mlook\u001b[0m to get a description of your surroundings
            \u001b[38;5;196mtake\u001b[0m to put an item in your pocket
            \u001b[38;5;196mitem\u001b[0m to use an item from your pockets

            Because of your injuries, you are rapidly bleeding out and must find
            time to rest and apply a bandage. Your health starts at 100 and will
            drop according to many things. Applying a bandage will increase your
            health by 25. Beware, however, that if you don't apply a bandage within
            5 minutes, you will die.
                """;
    public static String start = """
            Your eyes pry themselves open to nothing. You are enveloped in blackness
            on all sides. The only thing your senses can discern is an overwhelming
            potpourri of meat and death. Death is not a smell you're familiar with,
            but your instincts scream to you that the thick, unwelcoming atmosphere
            and earthy smell of blood is an omen for your own future.

            As your pupils dilate and your body reveals itself, you become conscious
            that your own blood is slowly dripping onto the concrete surface above
            your head. Your body fights your commands, involuntarily struggling
            and flailing like a mindless hooked fish. The aching limbs that you
            are now aware of clutch violently at the ropes suspending you upside-down
            from the ceiling in a desperate attempt to survive.

            Your broken body lands on the cold floor with a thud.
            The impact restores your senses.

            There is a door to your north and a small hatch to the west.
                """;
    public static String start2 = """
            You're in the room that you awoke in.
            It is still just as dark.

            There is a door to your north and a small corridor to the south.
                """;
    public static String leatherfaceStun = """
            \u001b[38;5;196mWith the sharp stinging noise of pierced flesh, the bone shard plunges into
            his neck.\u001b[0m The man shrieks. It is a horrible and inhuman wail, as if an
            otherworldly beast is attempting to express an emotion incomprehensible
            to the human mind.
                """;

    public static String boneUse = """
            You swipe wildly at the air in front of you for no apparent reason. Your
            brave assault results in the shard flying out of your hand and shattering
            on the ground. You wonder if you are still sane.
                """;
}
